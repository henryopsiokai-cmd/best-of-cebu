...
PRIVATE_KEY_FILE = 'temp_private_key_reindex.pem'
PARENT_FOLDERS = {
    "Week1": "1tIs2PcIj1LRBm4DNuyx9FrymBzuNbI4E",
    "Week2": "1YNmNDB3tKk8s9GnEqx_rgLlZdt5QoWOQ"
}
...
def list_all_files(token, folder_id):
    ...
    for f in files:
        if f['mimeType'] == 'application/vnd.google-apps.folder':
            all_files.extend(list_all_files(token, f['id']))
        else:
            all_files.append(f)
    return all_files
...
if __name__ == "__main__":
    try:
        ...
        index = {}
        for name, folder_id in PARENT_FOLDERS.items():
            index[name] = list_all_files(token, folder_id)
        
        with open('media_index.json', 'w') as f:
            json.dump(index, f, indent=2)
        print("Successfully indexed media assets.")
