...
WEEK1_FOLDER_ID = '1tIs2PcIj1LRBm4DNuyx9FrymBzuNbI4E'
...
if __name__ == "__main__":
    try:
        ...
        files = list_folder_contents(token, WEEK1_FOLDER_ID)
        # For each folder in files, list its contents
        for f in files:
            if f['mimeType'] == 'application/vnd.google-apps.folder':
                print(f"Folder: {f['name']} ({f['id']})")
                subfiles = list_folder_contents(token, f['id'])
                for sf in subfiles:
                    if sf['name'] == 'Photos' and sf['mimeType'] == 'application/vnd.google-apps.folder':
                        print(f"  Folder: Photos ({sf['id']})")
                        photos = list_folder_contents(token, sf['id'])
                        for p in photos:
                            print(f"    - {p['name']} ({p['id']})")
                    else:
