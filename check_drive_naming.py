...
PRIVATE_KEY_FILE = 'temp_private_key_heartbeat.pem'
WEEK2_FOLDER_ID = '1YNmNDB3tKk8s9GnEqx_rgLlZdt5QoWOQ'
...
if __name__ == "__main__":
    try:
        ...
        subfolders = list_folder_contents(token, WEEK2_FOLDER_ID)
        print(f"Checking {len(subfolders)} subfolders in Week 2...")
        for sf in subfolders:
            if sf['mimeType'] == 'application/vnd.google-apps.folder':
                print(f"\nFolder: {sf['name']} ({sf['id']})")
                files = list_folder_contents(token, sf['id'])
                for f in files:
                    print(f" - {f['name']}")
            else:
                print(f"File in root: {sf['name']}")
